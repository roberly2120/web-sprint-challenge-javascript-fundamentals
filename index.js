...
// nestedFunction has access to the variable 'internal' because of its lexical scope. Scope determines which items in a program have access to which other items. 'myFunction' has access to its own variables, but not those (if any) created within nestedFunction, while nestedFunction has access to all variables in itself and the outer 'myFunction'.
...
const zooAnimals = [
  ...
  */
  ...
  function lowerCaseNames(array){
    let lowerNames = [];
    array.map((element) => {
      lowerNames.push(element.animal_name.toLowerCase())
    });
  ...
  */
  ...
  function lowPopulationAnimals(array){
    const lowPop = array.filter((element, index) => {
      return element.population < 5
    });
    return lowPop;
  }
  // console.log(JSON.stringify(lowPopulationAnimals(zooAnimals)));
...
// const cuboid1 = new CuboidMaker({length: 5, width: 6, height: 9})
